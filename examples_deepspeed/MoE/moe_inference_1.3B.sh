...
do
    ...
    do
        ...
        do
            ...
            export PROFILER="nsys profile -o "${NAME}-${current_time}" -f true --capture-range=cudaProfilerApi --gpu-metrics-device=all"
            run_cmd="$PROFILER deepspeed --num_gpus $NUM_GPUS ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} &> ${OUTPUT_BASEPATH}/log/${NAME}-${current_time}.log"
            echo ${run_cmd}
            eval ${run_cmd}
            set +x
        done
